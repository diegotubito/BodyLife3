...
class Keychain {
    ...
    static func set(value: Data, service: KeychainService){
        let keychain = KeychainItem(service: service.rawValue, account: Bundle.main.bundleIdentifier!)
        try? keychain.saveItem(value)
    }
    
    //MARK: Get Keychain Object
    static func get(service: KeychainService) -> Data? {
        let keychain = KeychainItem(service: service.rawValue, account: Bundle.main.bundleIdentifier!)
        let data = try? keychain.readItem()
        return data
        
    }
    
    //MARK: Clear
    static func clear(service: KeychainService) {
        let keychain = KeychainItem(service: service.rawValue, account: Bundle.main.bundleIdentifier!)
...
fileprivate struct KeychainItem {
    ...
    func saveItem(_ value: Data?) throws {
        // Encode the password into an Data object.
        let encodedPassword = value
